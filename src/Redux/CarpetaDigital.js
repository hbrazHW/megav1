...
const ADJUNTO_EXITO = 'ADJUNTO_EXITO'
...
export default function documentosPorCuentaReducers(state = dataInicial, action){
    switch (action.type) {
        ...
        case CARGA_DOCUMENTOXCUENTA:
            return { ...state, cargaDocumento: action.cargaDocumento }
        case ADJUNTO_EXITO:
            return { ...state, adjuntos: action.payload }
        default:
            return { ...state }
    }
}
...
export const obtenerAdjuntoDocumentacionPorCuenta = (documentacionId) => async (dispatch) => {
    try {
        debugger;
        const entidad = 'new_documentacionporcuenta';
        const response = await axios.get(`${UrlApiDynamics}Notas?filter=objecttypecode eq '${entidad}' and _objectid_value eq ${documentacionId}&cuit=${Entidad}`)
        dispatch({
            type: ADJUNTO_EXITO,
            payload: response.data
        })
