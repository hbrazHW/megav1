...
import {
  ...
  cargarDocumentacionPorOperacion,
} from "../Redux/DocumentacionPorOperacion";
...
const Operaciones = () => {
  ...
  const documentosSelector = useSelector(
    (store) => store.documentos.documentos
  );
  const documentosPorOperacionSelector = useSelector(
    (store) => store.documentosOperacion.documentos
  );
  const resultadoOperacionSelector = useSelector(
    (store) => store.documentosOperacion.resultadoOperacion
  );
  const operacionesSelector = useSelector(
    (store) => store.operaciones.operaciones
  );
  const activo = useSelector((store) => store.usuarios.activo);
  const accountid = useSelector((store) => store.usuarios.accountid);
  const operacionIdSelector = useSelector(
    (store) => store.operaciones.operacionIdSeleccionada
  );
  ...
  React.useEffect(async () => {
    ...
    activo,
    operacionesSelector,
    documentosPorOperacionSelector,
    documentosSelector,
    operacionIdSelector,
  ]);
  ...
  const obtenerOperacionId = async (id) => {
    setOperacion(id);
    operaciones
      .filter((op) => op.new_operacionid === id)
      .map((item) => {
  ...
  const obtenerDocumentoXoperacion = async (e) => {
    setDocumento(e);
    documentos
      .filter((doc) => doc.new_documentacionid === e)
      .map((item) => {
                              ...
                              <div className="card pad color-header borde-none">
                                <div className="lista-header color-header pad">
                                  <div className="row p-3">
                                    <div className="col-sm-8">Operación</div>
                                    ...
                                    {documentosXOperacion
                                      ...
                                      .map((item) => {
                                        return (
                                          <li
                                            key={
                                              item.new_documentacionporoperacionid
                                            }
                              ...
                              <form onSubmit={ProcesarDocumentoOperacion}>
                                ...
                                <div class="mb-3">
                                  ...
                                  <select
                                    ...
                                    onChange={(e) =>
                                      obtenerDocumentoXoperacion(e.target.value)
                                    }
                                    value={documento}
                                    id="documento"
                                  >
                                ...
                                <div class="form-check form-switch">
                                  <input
                                    class="form-check-input switch-custom"
                                    type="checkbox"
