...
import {
  ...
  obtenerAdjuntoDocumentacionPorCuenta,
...
export const CarpetaDigital = () => {
  ...
  const [documentoAdjuntoId, setDocumentoAdjuntoId] = React.useState("");

  //Estados
  const documentosSelector = useSelector(
    (store) => store.documentosPorCuenta.documentos
  );
  const cargaDocumento = useSelector(
    (store) => store.documentosPorCuenta.cargaDocumento
  );
  const adjuntosSelector = useSelector(
    (store) => store.documentosPorCuenta.adjuntos
  );
  ...
  }, [activo, documentosSelector, cargaDocumento, adjuntosSelector]);
  ...
  const ObtenerAdjuntos = async (id) => {
    setAdjuntos([]);
    if (document.getElementById("spinner3") !== null) {
      document.getElementById("spinner3").hidden = false;
    }
    dispatch(obtenerAdjuntoDocumentacionPorCuenta(id));
  };
            ...
            <div className="card doc-cards pad borde-none pb-3">
              <div className="lista-header color-header pad borde-none">
                <div className="row p-3">
                  <div className="col-6">Documentación</div>
                  <div className="col-4 text-end">Estado</div>
                ...
                <ul className="list-group overflow-scroll lista-body">
                  {carpetas
                    .filter(
                      (carpeta) =>
                        carpeta.statuscode == 100000000 ||
                        carpeta.statuscode == 100000001
                    )
                    .map((item) => {
                      return (
                        <li
                          key={item.new_documentacionporcuentaid}
                          className="list-group-item h-100 p-0 pt-3 pb-3"
                        >
                          <div className="row d-flex align-items-center">
                            <div className="col-6">
                              <p className="text-lowercase m-0 fw-bolder mx-3 texto-lista">
                                {" "}
                                {item.new_name}
                              </p>
                            </div>
                            <div className="col-4 m-0 text-end">
                              <p className="text-lowercase m-0 fw-bolder mx-1 texto-lista">
                                {item.statuscode
                                  ? razonParaElEstado(item.statuscode)
                                  : "---"}
                              </p>
                            </div>
                              ...
                              <div className="dropdown">
                                ...
                                >
                                  <FontAwesomeIcon
                                    ...
                                    icon={faEllipsisH}
                                    className="fs-5 text-dark upload-file atras"
                                  />
                                </button>
                                <ul
          ...
          <div className="card shadow p-3 border-0 d-flex justify-content-start pad">
            ...
            <div className="card doc-cards pad borde-none">
              <div className="lista-header color-header pad">
                <div className="row p-3">
                  <div className="col-6">Documentación</div>
                  <div className="col-4 text-end p-0">Fecha Vencimiento</div>
                  <div className="col-2 text-end"></div>
                </div>
                ...
                <ul className="list-group w-100 overflow-auto lista-body">
                  {carpetas
                    .filter((carpeta) => carpeta.statuscode == 1)
                    .map((item) => {
                      return (
                        <li
                          key={item.new_documentacionporcuentaid}
                          className="list-group-item h-100 p-0 pt-1 pb-1"
                        >
                          <div className="row d-flex align-items-center">
                            ...
                            <div className="col-4 m-0 p-0 text-end">
                              <p className="text-lowercase m-0 fw-bolder text-end texto-lista">
                                {item.new_fechadevencimiento
                                  ? Moment(item.new_fechadevencimiento).format(
                                      "L"
                                    )
                                  : "-"}
                              </p>
                ...
                <div className="row p-3">
                  <div className="col-sm-12 color-header text-center c-azul fw-bolder">
                    {
                      carpetas.filter((carpeta) => carpeta.statuscode == 1)
                        .length
                    }{" "}
                    Documentos
                  </div>
          ...
          <div className="modal-content pad">
            ...
            <div className="modal-body">
              <div className="card doc-cards pad borde-none pb-3">
                <div className="lista-header color-header pad borde-none">
                  <div className="row p-3">
                    <div className="col-6">Documento</div>
                  </div>
