...
export const Lineas = () => {
  ...
  const [columnasLineasPorOperacion, setColumnasLineasPorOperacion] =
    React.useState([]);
    ...
    <animated.div className="container min-vh-100 mt-5" style={fade}>
      <div className="w-100">
        <div className="card shadow p-3 border-0 pad">
          ...
          {limites.length > 0 ? (
            <Tabla
              lineas={limites.filter(
                (limite) => limite.new_lineatipodeoperacion == 100000000
              )}
              columnas={columnasLineas}
              titulo={"Límites por Línea General"}
            />
          ) : (
            registros
          )}
        ...
        <div className="card shadow p-3 border-0 pad">
          ...
          {limites.length > 0 ? (
            <Tabla
              ...
              columnas={columnasLineasPorOperacion}
              titulo={"Límites por Operación Activos"}
            />
          ) : (
            registros
          )}
